...
class FlightSearchView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # 1. Генерируем уникальный ключ для кэша на основе параметров поиска
            cache_key = self._generate_cache_key(search_data)

            # 2. Пытаемся взять данные из Redis
            cached_data = cache.get(cache_key)
            if cached_data:
                return Response(cached_data, status=status.HTTP_200_OK)

            # 3. Если в кэше пусто — здесь будет вызов нашего Playwright сервиса
            parser = FlightParser()
            # Превращаем асинхронный запуск в обычный вызов
            found_flights = async_to_sync(parser.run)(search_data)

            # 4. Сохраняем результат в кэш на 30 минут
            # cache.set(cache_key, found_flights, timeout=60 * 30)
            cache.set(cache_key, found_flights, timeout=60 * 5)
